...
void ScriptEngine::InitMono()
{
	...
	MonoDomain* rootDomain = mono_jit_init("REngineJITRuntime");
	CORE_ASSERT(rootDomain, "");

	// Store the root domain pointer
	s_Data->RootDomain = rootDomain;

	// Create an App Domain
	s_Data->AppDomain = mono_domain_create_appdomain((char*)"RScriptRuntime", nullptr);
	mono_domain_set(s_Data->AppDomain, true);

	// Move this maybe
	s_Data->CoreAssembly = LoadCSharpAssembly("Resources/Scripts/RScript.dll");
	PrintAssemblyTypes(s_Data->CoreAssembly);

	MonoImage* assemblyImage = mono_assembly_get_image(s_Data->CoreAssembly);
	MonoClass* monoClass = mono_class_from_name(assemblyImage, "REngine", "Main");
