...
{
    if (RLGL.State.stackCounter >= RL_MAX_MATRIX_STACK_SIZE) CORE_ERROR("RLGL: Matrix stack overflow (RL_MAX_MATRIX_STACK_SIZE)");
...
void rlVertex3f(float x, float y, float z)
{
    ...
    else CORE_ERROR("RLGL: Batch elements overflow");
}
    ...
    switch (param)
    {
        ...
        case RL_TEXTURE_WRAP_T:
        {
            if (value == RL_TEXTURE_WRAP_MIRROR_CLAMP)
            {
                ...
                else CORE_WARN("GL: Clamp mirror wrap mode not supported (GL_MIRROR_CLAMP_EXT)");
#endif
            }
        ...
        case RL_TEXTURE_FILTER_ANISOTROPIC:
        {
            ...
            {
                CORE_WARN("GL: Maximum anisotropic filter level supported is {} {}", id, (int)RLGL.ExtSupported.maxAnisotropyLevel);
                glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, (float)value);
            }
            else CORE_WARN("GL: Anisotropic filtering not supported");
#endif
        } break;
        default: break;
    }
...
void rlActiveDrawBuffers(int count)
{
    ...
    if (count > 0)
    {
        if (count > 8) CORE_WARN("GL: Max color buffers limited to 8");
        else
        {
    ...
    else CORE_WARN("GL: One color buffer active by default");
#endif
}
...
void rlCheckErrors()
{
    ...
    while (check)
    {
        ...
        switch (err)
        {
            ...
            case 0x0500: CORE_WARN("GL: Error detected: GL_INVALID_ENUM"); break;
            case 0x0501: CORE_WARN("GL: Error detected: GL_INVALID_VALUE"); break;
            case 0x0502: CORE_WARN("GL: Error detected: GL_INVALID_OPERATION"); break;
            case 0x0503: CORE_WARN("GL: Error detected: GL_STACK_OVERFLOW"); break;
            case 0x0504: CORE_WARN("GL: Error detected: GL_STACK_UNDERFLOW"); break;
            case 0x0505: CORE_WARN("GL: Error detected: GL_OUT_OF_MEMORY"); break;
            case 0x0506: CORE_WARN("GL: Error detected: GL_INVALID_FRAMEBUFFER_OPERATION"); break;
            default: CORE_WARN("GL: Error detected: Unknown error code: {}", err); break;
        }
    }
#endif
}
...
static void GLAPIENTRY rlDebugMessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam)
{
    ...
    CORE_WARN("GL: OpenGL debug message: %s", message);
    CORE_WARN("    > Type: %s", msgType);
    CORE_WARN("    > Source = %s", msgSource);
    CORE_WARN("    > Severity = %s", msgSeverity);
}
#endif
...
void rlglInit(int width, int height)
{
    ...
    if (RLGL.State.defaultTextureId != 0) CORE_INFO("TEXTURE: [ID {}] Default texture loaded successfully", RLGL.State.defaultTextureId);
    else CORE_WARN("TEXTURE: Failed to load default texture");
    ...
    CORE_INFO("RLGL: Default OpenGL state initialized successfully");
    //----------------------------------------------------------
#endif
...
void rlglClose(void)
{
    ...
    CORE_INFO("TEXTURE: [ID {}] Default texture unloaded successfully", RLGL.State.defaultTextureId);
#endif
}
...
void rlLoadExtensions(void *loader)
{
    ...
    #if !defined(__APPLE__)
        if (gladLoadGL() == 0) CORE_WARN("GLAD: Cannot load OpenGL extensions");
        else CORE_INFO("GLAD: OpenGL extensions loaded successfully");
    #endif

    // Get number of supported extensions
    GLint numExt = 0;
    glGetIntegerv(GL_NUM_EXTENSIONS, &numExt);
    CORE_INFO("GL: Supported extensions count: {}", numExt);

#if defined(RLGL_SHOW_GL_DETAILS_INFO)
    // Get supported extensions list
    // WARNING: glGetStringi() not available on OpenGL 2.1
    CORE_INFO("GL: OpenGL extensions:");
    for (int i = 0; i < numExt; i++) CORE_INFO("    {}", glGetStringi(GL_EXTENSIONS, i));
#endif
...
#if defined(RLGL_SHOW_GL_DETAILS_INFO)
    CORE_INFO("GL: OpenGL extensions:");
    for (int i = 0; i < numExt; i++) CORE_INFO("    {}", extList[i]);
#endif
    ...
    CORE_INFO("GL: OpenGL device information:");
    ...
    CORE_INFO("GL: OpenGL capabilities:");
    GLint capability = 0;
    glGetIntegerv(GL_MAX_TEXTURE_SIZE, &capability);
    CORE_INFO("    GL_MAX_TEXTURE_SIZE: {}", capability);
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE, &capability);
    CORE_INFO("    GL_MAX_CUBE_MAP_TEXTURE_SIZE: {}", capability);
    glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &capability);
    CORE_INFO("    GL_MAX_TEXTURE_IMAGE_UNITS: {}", capability);
    glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &capability);
    CORE_INFO("    GL_MAX_VERTEX_ATTRIBS: {}", capability);
    #if !defined(GRAPHICS_API_OPENGL_ES2)
    glGetIntegerv(GL_MAX_UNIFORM_BLOCK_SIZE, &capability);
    CORE_INFO("    GL_MAX_UNIFORM_BLOCK_SIZE: {}", capability);
    glGetIntegerv(GL_MAX_DRAW_BUFFERS, &capability);
    CORE_INFO("    GL_MAX_DRAW_BUFFERS: {}", capability);
    if (RLGL.ExtSupported.texAnisoFilter) CORE_INFO("    GL_MAX_TEXTURE_MAX_ANISOTROPY: {}", RLGL.ExtSupported.maxAnisotropyLevel);
    #endif
    glGetIntegerv(GL_NUM_COMPRESSED_TEXTURE_FORMATS, &capability);
    CORE_INFO("    GL_NUM_COMPRESSED_TEXTURE_FORMATS: {}", capability);
    GLint *compFormats = (GLint *)RL_CALLOC(capability, sizeof(GLint));
    glGetIntegerv(GL_COMPRESSED_TEXTURE_FORMATS, compFormats);
    for (int i = 0; i < capability; i++) CORE_INFO("        {}", rlGetCompressedFormatName(compFormats[i]));
    RL_FREE(compFormats);

#if defined(GRAPHICS_API_OPENGL_43)
    glGetIntegerv(GL_MAX_VERTEX_ATTRIB_BINDINGS, &capability);
    CORE_INFO("    GL_MAX_VERTEX_ATTRIB_BINDINGS: {}", capability);
    glGetIntegerv(GL_MAX_UNIFORM_LOCATIONS, &capability);
    CORE_INFO("    GL_MAX_UNIFORM_LOCATIONS: {}", capability);
#endif  // GRAPHICS_API_OPENGL_43
#else   // RLGL_SHOW_GL_DETAILS_INFO

    // Show some basic info about GL supported features
    #if defined(GRAPHICS_API_OPENGL_ES2)
    if (RLGL.ExtSupported.vao) CORE_INFO("GL: VAO extension detected, VAO functions loaded successfully");
    else CORE_INFO("GL: VAO extension not found, VAO not supported");
    if (RLGL.ExtSupported.texNPOT) CORE_INFO("GL: NPOT textures extension detected, full NPOT textures supported");
    else CORE_INFO("GL: NPOT textures extension not found, limited NPOT support (no-mipmaps, no-repeat)");
    #endif
    if (RLGL.ExtSupported.texCompDXT) CORE_INFO("GL: DXT compressed textures supported");
    if (RLGL.ExtSupported.texCompETC1) CORE_INFO("GL: ETC1 compressed textures supported");
    if (RLGL.ExtSupported.texCompETC2) CORE_INFO("GL: ETC2/EAC compressed textures supported");
    if (RLGL.ExtSupported.texCompPVRT) CORE_INFO("GL: PVRT compressed textures supported");
    if (RLGL.ExtSupported.texCompASTC) CORE_INFO("GL: ASTC compressed textures supported");
    if (RLGL.ExtSupported.computeShader) CORE_INFO("GL: Compute shaders supported");
    if (RLGL.ExtSupported.ssbo) CORE_INFO("GL: Shader storage buffer objects supported");
#endif  // RLGL_SHOW_GL_DETAILS_INFO
...
rlRenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements)
{
    ...
    CORE_INFO("RLGL: Render batch vertex buffers loaded successfully in RAM (CPU)");
    //--------------------------------------------------------------------------------------------
    ...
    CORE_INFO("RLGL: Render batch vertex buffers loaded successfully in VRAM (GPU)");
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
    ...
    {
        CORE_WARN("GL: OpenGL 1.1 does not support GPU compressed texture formats");
        return id;
    }
#else
    ...
    {
        CORE_WARN("GL: DXT compressed texture format not supported");
        return id;
    }
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((!RLGL.ExtSupported.texCompETC1) && (format == RL_PIXELFORMAT_COMPRESSED_ETC1_RGB))
    {
        CORE_WARN("GL: ETC1 compressed texture format not supported");
        return id;
    }
    ...
    {
        CORE_WARN("GL: ASTC compressed texture format not supported");
        return id;
    }
#endif
    ...
    for (int i = 0; i < mipmapCount; i++)
    {
        ...
        CORE_DEBUG("TEXTURE: Load mipmap level {} ({} x {}), size: {}, offset: {}", i, mipWidth, mipHeight, mipSize, mipOffset);
    ...
    if (id > 0) CORE_INFO("TEXTURE: [ID {}] Texture loaded successfully ({}x{} | {} | {} mipmaps)", id, width, height, rlGetPixelFormatName(format), mipmapCount);
    else CORE_WARN("TEXTURE: Failed to load texture");
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer)
{
    ...
    if (!useRenderBuffer && RLGL.ExtSupported.texDepth)
    {
        ...
        CORE_INFO("TEXTURE: Depth texture loaded successfully");
    }
    else
    {
        ...
        CORE_INFO("TEXTURE: [ID {}] Depth renderbuffer loaded successfully ({} bits)", id, (RLGL.ExtSupported.maxDepthBits >= 24)? RLGL.ExtSupported.maxDepthBits : 16);
    }
#endif
...
unsigned int rlLoadTextureCubemap(const void *data, int size, int format)
{
            ...
            if (data == NULL)
            {
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
                {
                    if (format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32)
                    {
                        ...
                        else CORE_WARN("TEXTURES: Cubemap requested format not supported");
                    }
                    else if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32) || (format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32)) CORE_WARN("TEXTURES: Cubemap requested format not supported");
                    else glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, size, size, 0, glFormat, glType, NULL);
                }
                else CORE_WARN("TEXTURES: Empty cubemap creation does not support compressed format");
            }
            else
            {
    ...
    if (id > 0) CORE_INFO("TEXTURE: [ID {}] Cubemap texture loaded successfully ({}x{})", id, size, size);
    else CORE_WARN("TEXTURE: Failed to load cubemap texture");
...
void rlUpdateTexture(unsigned int id, int offsetX, int offsetY, int width, int height, int format, const void *data)
{
    ...
    else CORE_WARN("TEXTURE: [ID {}] Failed to update for current texture format ({})", id, format);
}
...
void rlGetGlTextureFormats(int format, int *glInternalFormat, int *glFormat, int *glType)
{
    ...
    *glType = -1;
    ...
    #endif
        default: CORE_WARN("TEXTURE: Current format not supported ({})", format); break;
    }
}
...
void rlGenTextureMipmaps(unsigned int id, int width, int height, int format, int *mipmaps)
{
    ...
    if (texIsPOT)
    {
        ...
        if (format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8)
        {
            ...
            *mipmaps = mipmapCount + 1;
            ...
            CORE_WARN("TEXTURE: [ID {}] Mipmaps generated manually on CPU side, total: {}", id, *mipmaps);
        }
        else CORE_WARN("TEXTURE: [ID {}] Failed to generate mipmaps for provided texture format", id);
    }
#endif
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((texIsPOT) || (RLGL.ExtSupported.texNPOT))
    {
        ...
        *mipmaps = 1 + (int)floor(log(MAX(width, height))/log(2));
        CORE_INFO("TEXTURE: [ID {}] Mipmaps generated automatically, total: {}", id, *mipmaps);
    }
#endif
    else CORE_WARN("TEXTURE: [ID {}] Failed to generate mipmaps", id);
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format)
{
    ...
    else CORE_WARN("TEXTURE: [ID {}] Data retrieval not suported for pixel format ({})", id, format);
...
bool rlFramebufferComplete(unsigned int id)
{
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE)
    {
        switch (status)
        {
            case GL_FRAMEBUFFER_UNSUPPORTED: CORE_WARN("FBO: [ID {}] Framebuffer is unsupported", id); break;
            case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: CORE_WARN("FBO: [ID {}] Framebuffer has incomplete attachment", id); break;
#if defined(GRAPHICS_API_OPENGL_ES2)
            case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS: CORE_WARN("FBO: [ID {}] Framebuffer has incomplete dimensions", id); break;
#endif
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: CORE_WARN("FBO: [ID {}] Framebuffer has a missing attachment", id); break;
            default: break;
        }
    }
...
void rlUnloadFramebuffer(unsigned int id)
{
    ...
    CORE_INFO("FBO: [ID {}] Unloaded framebuffer from VRAM (GPU)", id);
#endif
}
...
void rlUnloadVertexArray(unsigned int vaoId)
{
    ...
    if (RLGL.ExtSupported.vao)
    {
        ...
        CORE_INFO("VAO: [ID {}] Unloaded vertex array data from VRAM (GPU)", vaoId);
    }
#endif
}
...
unsigned int rlLoadShaderCode(const char *vsCode, const char *fsCode)
{
    ...
    else
    {
        ...
        if (id == 0)
        {
            ...
            CORE_WARN("SHADER: Failed to load custom shader code, using default shader");
            id = RLGL.State.defaultShaderId;
        }
        /*
        else
...
unsigned int rlCompileShader(const char *shaderCode, int type)
{
    ...
    if (success == GL_FALSE)
    {
        switch (type)
        {
            case GL_VERTEX_SHADER: CORE_WARN("SHADER: [ID {}] Failed to compile vertex shader code", shader); break;
            case GL_FRAGMENT_SHADER: CORE_WARN("SHADER: [ID {}] Failed to compile fragment shader code", shader); break;
            //case GL_GEOMETRY_SHADER:
        #if defined(GRAPHICS_API_OPENGL_43)
            case GL_COMPUTE_SHADER: CORE_WARN("SHADER: [ID {}] Failed to compile compute shader code", shader); break;
        #endif
            default: break;
        }
        ...
        if (maxLength > 0)
        {
            ...
            CORE_WARN("SHADER: [ID {}] Compile error: {}", shader, log);
            RL_FREE(log);
        }
    }
    else
    {
        switch (type)
        {
            case GL_VERTEX_SHADER: CORE_INFO("SHADER: [ID {}] Vertex shader compiled successfully", shader); break;
            case GL_FRAGMENT_SHADER: CORE_INFO("SHADER: [ID {}] Fragment shader compiled successfully", shader); break;
            //case GL_GEOMETRY_SHADER:
        #if defined(GRAPHICS_API_OPENGL_43)
            case GL_COMPUTE_SHADER: CORE_INFO("SHADER: [ID {}] Compute shader compiled successfully", shader); break;
        #endif
            default: break;
        }
    }
...
unsigned int rlLoadShaderProgram(unsigned int vShaderId, unsigned int fShaderId)
{
    ...
    if (success == GL_FALSE)
    {
        CORE_WARN("SHADER: [ID {}] Failed to link shader program", program);
        ...
        if (maxLength > 0)
        {
            ...
            CORE_INFO("SHADER: [ID {}] Link error: {}", program, log);
            RL_FREE(log);
        }
    ...
    else
    {
        ...
        CORE_INFO("SHADER: [ID {}] Program shader loaded successfully", program);
    }
#endif
    return program;
}
...
void rlUnloadShaderProgram(unsigned int id)
{
    ...
    CORE_INFO("SHADER: [ID {}] Unloaded shader program data from VRAM (GPU)", id);
#endif
}
...
int rlGetLocationUniform(unsigned int shaderId, const char *uniformName)
{
    ...
    if (location == -1) CORE_WARN("SHADER: [ID {}] Failed to find shader uniform: {}", shaderId, uniformName);
    else CORE_INFO("SHADER: [ID {}] Shader uniform ({}) set at location: {}", shaderId, uniformName, location);
#endif
    return location;
}
...
int rlGetLocationAttrib(unsigned int shaderId, const char *attribName)
{
    ...
    if (location == -1) CORE_WARN("SHADER: [ID {}] Failed to find shader attribute: {}", shaderId, attribName);
    else CORE_INFO("SHADER: [ID {}] Shader attribute ({}) set at location: {}", shaderId, attribName, location);
#endif
    return location;
}
...
void rlSetUniform(int locIndex, const void *value, int uniformType, int count)
{
    ...
    switch (uniformType)
    {
        ...
        default: CORE_WARN("SHADER: Failed to set uniform value, data type not recognized");
    }
#endif
}
...
void rlSetVertexAttributeDefault(int locIndex, const void *value, int attribType, int count)
{
    ...
    switch (attribType)
    {
        ...
        default: CORE_WARN("SHADER: Failed to set attrib default value, data type not recognized");
    }
#endif
}
...
unsigned int rlLoadComputeShaderProgram(unsigned int shaderId)
{
    ...
    if (success == GL_FALSE)
    {
        CORE_WARN("SHADER: [ID {}] Failed to link compute shader program", program);
        ...
        if (maxLength > 0)
        {
            ...
            glGetProgramInfoLog(program, maxLength, &length, log);
            CORE_WARN("SHADER: [ID {}] Link error: {}", program, log);
            RL_FREE(log);
        }
    ...
    else
    {
        ...
        CORE_INFO("SHADER: [ID {}] Compute shader program loaded successfully", program);
    }
#endif
...
static void rlLoadShaderDefault(void)
{
    ...
    {
        CORE_INFO("SHADER: [ID {}] Default shader loaded successfully", RLGL.State.defaultShaderId);
    ...
    else CORE_WARN("SHADER: [ID {}] Failed to load default shader", RLGL.State.defaultShaderId);
}
...
static void rlUnloadShaderDefault(void)
{
    ...
    CORE_INFO("SHADER: [ID {}] Default shader unloaded successfully", RLGL.State.defaultShaderId);
}
...
static int rlGenTextureMipmapsData(unsigned char *data, int baseWidth, int baseHeight)
{
    ...
    while ((width != 1) && (height != 1))
    {
        ...
        CORE_DEBUG("TEXTURE: Next mipmap size: {} x {}", width, height);
    ...
    CORE_DEBUG("TEXTURE: Total mipmaps required: {}", mipmapCount);
    CORE_DEBUG("TEXTURE: Total size of data required: {}", size);

    unsigned char *temp = RL_REALLOC(data, size);

    if (temp != NULL) data = temp;
    else CORE_WARN("TEXTURE: Failed to re-allocate required mipmaps memory");
    ...
    CORE_DEBUG("TEXTURE: Mipmap base size ({}x{})", width, height);
...
static unsigned char *rlGenNextMipmapData(unsigned char *srcData, int srcWidth, int srcHeight)
{
    ...
    CORE_DEBUG("TEXTURE: Mipmap generated successfully ({}x{})", width, height);
